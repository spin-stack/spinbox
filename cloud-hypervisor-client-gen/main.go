// Package main implements a code generator that creates a Go client for the Cloud Hypervisor API
// by parsing its OpenAPI specification. It generates type definitions and client methods
// for interacting with the Cloud Hypervisor API endpoints.
package main
...
func main() {
	version := os.Getenv("CH_VERSION")
	if version == "" {
		version = "v49.0"
	}
	url := fmt.Sprintf("https://raw.githubusercontent.com/cloud-hypervisor/cloud-hypervisor/%s/vmm/src/api/openapi/cloud-hypervisor.yaml", version)
...
func newEndpoint(path string, pathItem *openapi.PathItem) *endpoint {
	name := strings.TrimPrefix(strcase.ToCamel(path), "vm")
