...
import (
	...
	"github.com/aledbf/beacon/containerd/internal/bufpool"
	"github.com/aledbf/beacon/containerd/internal/vminit/process"
...
func (p *linuxPlatform) CopyConsole(ctx context.Context, console console.Console, id, stdin, stdout, stderr string, wg *sync.WaitGroup) (cons console.Console, retErr error) {
	...
	if stdin != "" {
		...
		go func() {
			cwg.Done()
			bp := bufpool.Pool.Get().(*[]byte)
			defer bufpool.Pool.Put(bp)
